...
Message EcrManager::ProcessTransaction(const Message& message) {
  ...
  switch (*eft_txn_type) {
    ...
    case EftTxnType::TT_VOID: {
      auto invoice_num =  GetEftDataField(message);
      if (!invoice_num.has_value())
        return GetTxInvalidResponse();
      tx_uptr = ProcessVoid(*invoice_num);
      break;
    }
    default:
      break;
...
std::unique_ptr<Transaction> EcrManager::ProcessVoid(const std::string& invoice_num) {
  return acquirer_.StartVoid(invoice_num);
}
