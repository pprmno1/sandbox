...
MenuManager::MenuManager(ApplicationSettings& app_setting,
  ...
  if (app_setting_.managed_settings_->terminal_config.TipProcessing()
      != fdms::TerminalConfiguration::TIP_DISABLED)
    main_menu_.push_back(
  ...
  if (app_setting_.managed_settings_->terminal_config.TxQuasiCashEnabled())
    main_menu_.push_back(
        std::make_pair("QUASI CASH", [&]() {return acquirer_.StartQuasiCash();}));
  ...
  main_menu_.push_back(
        std::make_pair(
            "TELIUM MANAGER",
            [&]() {return acquirer_.StartManagerMenu();}));
  ...
  if (app_setting_.managed_settings_->terminal_config.TxMotoPreauthEnabled())
    moto_menu_.push_back(
  ...
  if (app_setting_.managed_settings_->terminal_config.TxPreAuthCompletionOfflineEnabled())
    preauth_menu_.push_back(
        std::make_pair("PRE-AUTH COMP OFFLINE",
                       [&]() {return acquirer_.StartCompletionOffline();}));
  if (app_setting_.managed_settings_->terminal_config.TxPreAuthCompletionOnlineEnabled())
    preauth_menu_.push_back(
        std::make_pair("PRE-AUTH COMP",
                       [&]() {return acquirer_.StartCompletionOnline();}));
  if (app_setting_.managed_settings_->terminal_config.TxVoidPreAuthEnabled())
    preauth_menu_.push_back(
...
//Should be enabled only if fdms requires otherwise use void for installment cancellation
  if (false)  // instalment void enable
    instalment_menu_.push_back(
  ...
  admin_function_menu_ = {
    ...
    { "CLEAR BATCH        +", [&]() {return acquirer_.StartClearBatch();}},
    { "TMK DOWNLOAD       +", [&]() {return acquirer_.StartTMKDownload();}},
    { "LOGGER             +", [&]() {return StartLoggerConfig(ui_);}}};
  ...
  merchant_menu = {
    ...
    { "LOGON/KEY EXCHANGE  ", [&]() {return acquirer_.StartKeyExchange();}},
    { "LOCK TERMINAL       ", [&]() {app_setting_.local_settings_->IsTerminalLocked(true);
                                      ui_.MessageTerminalLocked();
                                      return false;}}
  };
